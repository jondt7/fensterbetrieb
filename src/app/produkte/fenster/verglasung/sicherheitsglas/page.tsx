    ...
    <div className="mx-auto max-w-7xl px-4 py-10 md:py-14">
      ...
      <section className="mb-12 rounded-2xl border border-zinc-200 bg-white p-8">
        ...
        <ul className="mt-3 grid gap-2 text-sm text-zinc-700 md:grid-cols-2">
          {notes.map((n) => (
            <li key={n} className="rounded-lg bg-brand-light px-3 py-2">• {n}</li>
          ))}
        </ul>
      </section>
      ...
      <section className="rounded-2xl border border-brand bg-white p-8 shadow-sm">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <Link href="/produkte/fenster/beschlag-sicherheit" className="inline-flex items-center rounded-xl border border-brand-dark px-5 py-3 text-sm font-semibold text-brand-dark hover:bg-brand-light">Beschlag &amp; Sicherheit</Link>
